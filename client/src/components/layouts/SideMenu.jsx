  ...
  return (
    <div className='w-64 h-[calc(100vh-61px)] bg-white border-r border-gray-200/50 p-5 sticky top-15.25 z-20'>
      ...
      {SIDE_MENU_DATA.map((item, index) => (
        <button onClick={() => handleClick(item.path)} key={`menu_${index}`} className={`w-full flex items-center gap-4 text-[15px] py-3 px-6 rounded-lg mb-3 ${activeMenu == item.label ? "text-white bg-primary" : ""}`}>
